...
- Which transport serves the Sonnet-class roles (Judge, second proposal sample, Reviewer sign-off, Reflector). **Decided: OpenRouter `anthropic/claude-sonnet-5` with the project's own key** — no new credential, cost metered in the same ledger path, about $0.04 per Judge call and roughly $9 a month for the whole role set. Consequence for Task 16: the addendum's `Harness` enum records the role's model tier, not its transport — `CLAUDE_NATIVE` means the role runs on the Claude model family (currently served over OpenRouter), `HERMES_OPENROUTER` means a cheap OpenRouter model. One adapter therefore serves every role, with the model per role in `config/models.yaml`. Never Opus, and a config naming one must fail validation at startup.
- Who owns the Telegram receive path. **Decided: Hermes relays inbound commands to the project CLI.** The Hermes gateway already polls the bot token with getUpdates and Telegram allows one polling consumer per token, so the project's own `telegram_bot.py` must never poll. Outbound: the project sends cards and reports through the bot API (no polling) or `hermes send`. Inbound: Hermes receives `/veto`, `/pause`, `/resume`, `/status`, `/why`, `/flat`, `/fund approve|decline` and hands each to the project CLI (`agentoquant veto --card-id X`, …) through a Hermes webhook subscription or a small plugin command; the project's 10-minute veto window waits on its own signal store. This interface must exist before Task 21's veto timer is built.
...
- Telegram: the project reuses the existing Hermes bot. Outbound goes through the bot API without polling; inbound control commands are received by the Hermes gateway and relayed to the project CLI (the token has exactly one poller and it is Hermes).
- Sonnet-class roles (Judge, second proposal sample, Reviewer sign-off, Reflector) are served by OpenRouter `anthropic/claude-sonnet-5` on the project's own key, so every model call in the pipeline is metered in one place. The `Harness` enum records the role's model tier, not the transport.
