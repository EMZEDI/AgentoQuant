...
- Usage data comes from DefiLlama's free API (fees, revenue, TVL, DEX volumes; Venice and Akash have fee adapters, Bittensor and Render are thin) and token unlocks from CryptoRank's free Sandbox API (about 378 tokens, daily resolution), because DefiLlama's unlocks moved to its $300 a month Pro tier. CryptoPanic's free API was discontinued in early 2026 and is optional paid only.
...
- Verified in September 2026: DefiLlama free API (fees, revenue, TVL, DEX volumes, keyless, about 500 requests per minute; unlocks are Pro only at $300 a month); CryptoRank Sandbox API (free, 10k credits a month, 10 requests per minute, vesting and unlock endpoints included); Grok X Search through OpenRouter's native web plugin ($5 per 1k posts from Sept 21, 2026, plus Grok tokens); CryptoPanic free API discontinued; Whale Alert paid only; Kraken OHLC endpoint limited to 720 candles.
