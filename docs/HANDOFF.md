...
## Open decisions that block Task 16 and Task 21

These are unresolved in the spec. Do not invent an answer — the phase agent asks Shahrad.

1. **Transport for the Sonnet-class roles (Judge, second proposal sample, Reviewer sign-off, Reflector).** The plan names a "Sonnet-class Claude" but not how it is reached. Recommended: OpenRouter `anthropic/claude-sonnet-5` ($2/$10 per 1M, verified available, no new credential, cost metered in the same ledger path as every other call) — a Judge call is ~$0.04, and the whole Sonnet role set is ~$9/month. Alternatives: direct Anthropic API (needs a key the box does not have) or Claude Code CLI (not installed). Consequence for Task 16: if Sonnet arrives over OpenRouter, the addendum's `Harness` enum must be read as *model family*, not transport, so one adapter serves every role.
2. **Who owns the Telegram receive path.** The Hermes gateway already polls the bot token with `getUpdates`, and Telegram allows exactly one polling consumer — so the project's own `execution/telegram_bot.py` cannot also poll it without 409 conflicts and dropped messages. Recommended: the project **sends** cards and reports through the bot API (no polling) and Hermes **receives** `/veto`, `/pause`, `/resume`, `/status`, `/why`, `/flat`, `/fund approve|decline` and passes each to the project CLI (`agentoquant veto --card-id X`, …), via a Hermes webhook subscription or a small plugin command. Alternative: a dedicated bot token for the project, which avoids the shared-token coupling entirely. Consequence for Task 21: the inbound path needs to be specified before the veto timer is built.
